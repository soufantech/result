...
test('mapFailureAsync maps a failure with mapFn returing a Promise with a value.', async () => {
  ...
  const result = await failure<string, Error>(error).mapFailureAsync((s) =>
    Promise.resolve(s).then(x),
  );
...
test('mapFailureAsync maps a success bypassing mapFn.', async () => {
  const result = await success<string, Error>('ay').mapFailureAsync((e) =>
    Promise.resolve(e).then(x),
  );
