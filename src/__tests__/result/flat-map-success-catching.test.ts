...
test('flatMapSuccessCatching returns the Result instance returned by the mapping function.', () => {
...
test('flatMapSuccessCatching returns the Result instance returned by the mapping function catching.', () => {
  const result = success(771)
    .flatMapSuccessCatching((n) => success(f(String(n))))
...
test('flatMapSuccessCatching forwards a failure result.', () => {
