...
test('recoverCatching has no effect on a success Result.', () => {
...
test('recoverCatching returns the value returned from the transform function in a new Result when invoked on failure Result.', () => {
...
test('recoverCatching returns a failure Result with the Error instance thrown by the transform function invoked on failure Result.', () => {
  const err = new Error('nay');
